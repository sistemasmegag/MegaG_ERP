...
function inv_schema(): string
{
    return mg_schema();
}

function inv_table(string $object): string
{
    return mg_table($object);
}

function inv_sequence(string $object): string
{
    return mg_sequence($object);
}
...
function inv_throw_if_missing_schema(PDO $conn): void
{
    ...
    foreach ($requiredTables as $tableName) {
        try {
            $conn->query("SELECT 1 FROM " . inv_table($tableName) . " WHERE 1 = 0");
        } catch (Throwable $e) {
            throw new Exception(
                'Não foi possível acessar ' . inv_schema() . '.' . $tableName .
...
function inv_fetch_request_form_domains(PDO $conn): array
{
    ...
    $stmt = $conn->prepare("SELECT CENTRORESULTADO AS CENTROCUSTO,
                                   SEQCENTRORESULTADO,
                                   DESCRICAO AS NOME
                              FROM " . inv_table('ABA_CENTRORESULTADO') . "
                             ORDER BY DESCRICAO");
    $stmt->execute();
...
function inv_fetch_responsibles_by_cost_center(PDO $conn, string $costCenterValue): array
{
    ...
    $stmt = $conn->prepare("SELECT DISTINCT A.SEQUSUARIO,
                                   A.NOME,
                                   A.CODGRUPO
                              FROM " . inv_table('MEGAG_DESP_APROVADORES') . " A
...
function inv_format_dashboard(PDO $conn): array
{
    $sql = "SELECT COUNT(*) AS TOTAL,
              ...
              FROM " . inv_table('MEGAG_TI_EQUIPAMENTOS') . "
             WHERE ATIVO = 'S'";
    $stmt = $conn->query($sql);
    ...
    if ($type === 'TERMOS_ASSINADOS') {
        ...
        $rows = inv_report_rows(
            ...
            "SELECT t.ID_TERMO,
               ...
               FROM " . inv_table('MEGAG_TI_TERMOS') . " t
               JOIN " . inv_table('MEGAG_TI_EQUIPAMENTOS') . " e
...
{
    $idHistorico = inv_nextval($conn, inv_sequence('SEQ_MEGAG_TI_EQUIP_HIST'));
    $options = inv_options();
    $resumo = $tipo === 'CADASTRO'
        ? 'Equipamento cadastrado no inventário.'
        : inv_build_resume($before, $after);

    $stmt = $conn->prepare("INSERT INTO " . inv_table('MEGAG_TI_EQUIP_HIST') . " (
                                ID_HISTORICO,
                                ID_EQUIPAMENTO,
                                TIPO_MOVIMENTACAO,
                                STATUS_ANTERIOR,
...
function inv_save(PDO $conn, array $payload, string $user): array
{
    ...
    try {
        if ($payload['id_equipamento']) {
            ...
            $stmt = $conn->prepare("UPDATE " . inv_table('MEGAG_TI_EQUIPAMENTOS') . "
            ...
            $idEquipamento = inv_nextval($conn, inv_sequence('SEQ_MEGAG_TI_EQUIPAMENTOS'));

            $stmt = $conn->prepare("INSERT INTO " . inv_table('MEGAG_TI_EQUIPAMENTOS') . " (
                                        ID_EQUIPAMENTO,
                                        COD_PATRIMONIO,
                                        NOME_EQUIPAMENTO,
                                        TIPO,
...
function inv_history(PDO $conn, int $id): array
{
    ...
    $stmt = $conn->prepare("SELECT ID_HISTORICO,
                              ...
                              FROM " . inv_table('MEGAG_TI_EQUIP_HIST') . "
                             WHERE ID_EQUIPAMENTO = :ID
...
function inv_get_term(PDO $conn, int $idEquipamento): array
{
    ...
    $stmt = $conn->prepare("SELECT ID_TERMO,
                                   ...
                                   CRIADO_POR,
                                   TO_CHAR(CRIADO_EM, 'DD/MM/YYYY HH24:MI') AS CRIADO_EM_FMT
                              FROM " . inv_table('MEGAG_TI_TERMOS') . "
                             WHERE ID_EQUIPAMENTO = :ID
...
function inv_save_term(PDO $conn, array $payload, string $user): array
{
    ...
    $idTermo = inv_nextval($conn, inv_sequence('SEQ_MEGAG_TI_TERMOS'));

    $conn->beginTransaction();
    try {
        $stmt = $conn->prepare("INSERT INTO " . inv_table('MEGAG_TI_TERMOS') . " (
                                    ID_TERMO,
                                    ID_EQUIPAMENTO,
                                    NOME_COLABORADOR,
                                    CPF_COLABORADOR,
...
function inv_list_requests(PDO $conn): array
{
    $stmt = $conn->query("SELECT ID_SOLICITACAO,
                                 ...
                                 TO_CHAR(CRIADO_EM, 'DD/MM/YYYY HH24:MI') AS CRIADO_EM_FMT
                            FROM " . inv_table('MEGAG_ALMOX_SOLICITACOES') . "
...
function inv_save_request(PDO $conn, array $payload, string $user): array
{
    ...
    try {
        if ($payload['id_solicitacao']) {
            $stmt = $conn->prepare("UPDATE " . inv_table('MEGAG_ALMOX_SOLICITACOES') . "
            ...
            $idSolicitacao = inv_nextval($conn, inv_sequence('SEQ_MEGAG_ALMOX_SOLICITACOES'));
            $stmt = $conn->prepare("INSERT INTO " . inv_table('MEGAG_ALMOX_SOLICITACOES') . " (
                                        ID_SOLICITACAO,
                                        SOLICITANTE_NOME,
                                        SOLICITANTE_LOGIN,
                                        SOLICITANTE_CPF,
