...
function desp_find_recent_duplicate(
    ...
    $st->bindValue(':SEQCC', $seqCentroResultado, PDO::PARAM_INT);
...
function process_approval_action(PDO $conn, int $codDespesa, int $seqUsuario, string $status, string $pago, string $observacao): string
{
    ...
    try {
        ...
        $stNivelGlobal = $conn->prepare(mg_with_schema("
            SELECT MIN(NIVEL_APROVACAO)
              FROM CONSINCO.MEGAG_DESP_APROVACAO
             WHERE CODDESPESA = :ID
               AND STATUS = 'LANCADO'
        ...
        $stNivelUsuario = $conn->prepare(mg_with_schema("
            SELECT MIN(NIVEL_APROVACAO)
              FROM CONSINCO.MEGAG_DESP_APROVACAO
             WHERE CODDESPESA = :ID
        ...
        $stUpdate = $conn->prepare(mg_with_schema("
        ...
        if ($status === 'REJEITADO') {
            $stDespRej = $conn->prepare(mg_with_schema("
        ...
        $stRestantes = $conn->prepare(mg_with_schema("
            SELECT COUNT(*)
              FROM CONSINCO.MEGAG_DESP_APROVACAO
             WHERE CODDESPESA = :ID
               AND STATUS = 'LANCADO'
        ...
        if ($restantes === 0) {
            $stDespApr = $conn->prepare(mg_with_schema("
                UPDATE CONSINCO.MEGAG_DESP
                   SET STATUS = 'APROVADO',
                       PAGO = :PAGO,
                       DTAALTERACAO = SYSDATE
        ...
        $stDespFluxo = $conn->prepare(mg_with_schema("
            UPDATE CONSINCO.MEGAG_DESP
        ...
        $stNext = $conn->prepare(mg_with_schema("
...
try {
    ...
    if ($action === 'get_doms') {
        ...
        $stCc = $conn->prepare(mg_with_schema("SELECT CENTRORESULTADO AS CENTROCUSTO, SEQCENTRORESULTADO, DESCRICAO AS NOME FROM CONSINCO.ABA_CENTRORESULTADO ORDER BY DESCRICAO"));
        $stCc->execute();
    ...
    if ($action === 'list_approvals') {
        ...
        foreach ($rows as &$r) {
            ...
            $stCc = $conn->prepare(mg_with_schema("SELECT DESCRICAO, CENTRORESULTADO FROM CONSINCO.ABA_CENTRORESULTADO WHERE SEQCENTRORESULTADO = :C AND ROWNUM = 1"));
