...
if (!empty($_SERVER['DOCUMENT_ROOT'])) {
    $dbConnectCandidates[] = rtrim($_SERVER['DOCUMENT_ROOT'], '/\\') . '/db_config/db_connect.php';
}
$dbConnectCandidates[] = dirname(__DIR__, 2) . '/db_config/db_connect.php';
$dbConnectCandidates[] = dirname(__DIR__, 2) . '/config/db_connect.php';

$dbConnectPath = null;
foreach ($dbConnectCandidates as $candidatePath) {
    if (is_string($candidatePath) && $candidatePath !== '' && file_exists($candidatePath)) {
        $dbConnectPath = $candidatePath;
        break;
    }
}
