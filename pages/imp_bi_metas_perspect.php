<?php
$paginaAtual = 'bi_metas_perspect';
?>

<style>
/* ===== Clean SaaS (escopado pra página BI_METAS_PERSPECT) ===== */
.saas-head{
    border: 1px solid var(--saas-border);
    background: linear-gradient(135deg, rgba(255,193,7,.14), rgba(255,193,7,.05));
    border-radius: 18px;
...
html[data-theme="dark"] .saas-head{
    background: linear-gradient(135deg, rgba(255,193,7,.16), rgba(255,255,255,.02));
}
.saas-head:before{
    content:"";
    position:absolute;
    ...
    width: 360px;
    height: 360px;
    background: radial-gradient(circle at 30% 30%, rgba(255,193,7,.32), transparent 60%);
    filter: blur(6px);
...
.saas-upload .btn{
    ...
    font-weight: 900;
    letter-spacing: .02em;
    box-shadow: 0 10px 18px rgba(255,193,7,.22);
}
      ...
      <div class="col-lg-10 col-xl-9">
        ...
        <div class="saas-head mb-4">
          <div class="d-flex flex-wrap align-items-center justify-content-between gap-3 position-relative">
            <div>
              <h3 class="saas-title">Importação BI Metas Perspectiva</h3>
              <p class="saas-subtitle">
                Faça o upload da planilha com
                <strong>CODMETA, PERSPEC, DATA, STATUS, ATUALIZACAO</strong>.
              </p>
            </div>

            <div class="d-flex align-items-center gap-2">
              <span class="badge rounded-pill bg-warning bg-opacity-10 text-muted fw-bold px-3 py-2">
                Destino: Oracle
              </span>
            </div>
          </div>
        ...
        <div class="card saas-card mb-4">
          <div class="card-header py-3 d-flex justify-content-between align-items-center">
            <div class="d-flex align-items-center gap-2">
              <div class="bg-warning bg-opacity-10 rounded-3 d-flex align-items-center justify-content-center" style="width:42px;height:42px;">
                <i class="bi bi-diagram-3 fs-4 text-warning"></i>
              </div>
              <div>
                <div class="saas-kicker">Upload</div>
                <div class="fw-bold text-dark" style="letter-spacing:-.01em;">BI Metas Perspectiva</div>
              </div>
            </div>
          ...
          <div class="card-body">
            ...
            <div class="input-group input-group-lg mb-2 saas-upload">
              ...
              <button class="btn btn-warning px-4" type="button" onclick="iniciar()" id="btnGo">
                <i class="bi bi-play-fill me-1"></i> IMPORTAR TABELA
              </button>
            </div>

            <small class="text-muted">
              Colunas esperadas: CODMETA | PERSPEC | DATA | STATUS | ATUALIZACAO.
            </small>
          </div>
        </div>
...
<script>
    ...
    async function iniciar() {
        ...
        try {
            ...
            const evt = new EventSource(`processors/processa_universal_insert.php?tipo=bi_metas_perspect&arquivo=${json.arquivo}`);
