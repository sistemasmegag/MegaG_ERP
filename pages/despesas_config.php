...
<script>
    ...
    async function abrirEdicaoPolitica(encodedRow) {
        ...
        const ccSeq = row.CENTROCUSTO || row.SEQCENTRORESULTADO || '';
        const ccValue = `${ccCodigo}|${ccSeq}`;
    ...
    async function salvarConfig() {
        ...
        if (modalMode === 'Categoria') {
            ...
            const aprovadores = collectPoliticaAprovadores('fPolAprovadores');
            if (!aprovadores.length) {
                alert('Selecione um aprovador para a polÃ­tica.');
                return;
            }
            payload = {
                action: 'update_politica',
                codpolit_cc: editingPolitica?.CODPOLIT_CC || editingPolitica?.codpolit_cc || 0,
                codgrupo: document.getElementById('fPolGrupo').value,
                centro_custo: document.getElementById('fPolCc').value,
