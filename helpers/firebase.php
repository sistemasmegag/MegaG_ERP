...
function mg_firebase_config(): array
{
    ...
    $serviceAccount = [];
    $serviceAccountPath = trim((string)($localConfig['service_account_json_path'] ?? getenv('FIREBASE_SERVICE_ACCOUNT_JSON_PATH') ?? ''));
    ...
    $enabledValue = $localConfig['enabled'] ?? getenv('FIREBASE_ENABLED') ?? false;
    $enabled = filter_var($enabledValue, FILTER_VALIDATE_BOOLEAN);

    $privateKey = (string)($localConfig['private_key'] ?? getenv('FIREBASE_PRIVATE_KEY') ?? ($serviceAccount['private_key'] ?? ''));
    $privateKey = str_replace(["\r\n", '\n'], ["\n", "\n"], $privateKey);

    $config = [
        'enabled' => $enabled,
        'project_id' => trim((string)($localConfig['project_id'] ?? getenv('FIREBASE_PROJECT_ID') ?? ($serviceAccount['project_id'] ?? ''))),
        'api_key' => trim((string)($localConfig['api_key'] ?? getenv('FIREBASE_API_KEY') ?? '')),
        'app_id' => trim((string)($localConfig['app_id'] ?? getenv('FIREBASE_APP_ID') ?? '')),
        'messaging_sender_id' => trim((string)($localConfig['messaging_sender_id'] ?? getenv('FIREBASE_MESSAGING_SENDER_ID') ?? ($serviceAccount['project_number'] ?? ''))),
        'auth_domain' => trim((string)($localConfig['auth_domain'] ?? getenv('FIREBASE_AUTH_DOMAIN') ?? '')),
        'storage_bucket' => trim((string)($localConfig['storage_bucket'] ?? getenv('FIREBASE_STORAGE_BUCKET') ?? '')),
        'measurement_id' => trim((string)($localConfig['measurement_id'] ?? getenv('FIREBASE_MEASUREMENT_ID') ?? '')),
        'vapid_key' => trim((string)($localConfig['vapid_key'] ?? getenv('FIREBASE_VAPID_KEY') ?? '')),
        'service_account_email' => trim((string)($localConfig['service_account_email'] ?? getenv('FIREBASE_SERVICE_ACCOUNT_EMAIL') ?? ($serviceAccount['client_email'] ?? ''))),
        'private_key' => trim($privateKey),
        'token_uri' => trim((string)($localConfig['token_uri'] ?? getenv('FIREBASE_TOKEN_URI') ?? ($serviceAccount['token_uri'] ?? 'https://oauth2.googleapis.com/token'))),
