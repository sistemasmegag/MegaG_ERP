...
try {
    ...
    // ============================================================
    $importTableBase = 'MEGAG_IMP_METAS';
    ...
    $required = [
        ...
        'DTAATUALIZACAO' => ['DTAATUALIZACAO','DTA_ATUALIZACAO','DTATUALIZACAO','DATAATUALIZACAO','ATUALIZACAO'],
    ];
    ...
    // chaves do merge
    $mergeKeys = ['CODMETA','CODVENDEDOR','CODREGIAO','SEQPRODUTO'];
    ...
    $updCols = ['META','SEGMENTO','TIPORETIRA','CATEGORIA','DTAATUALIZACAO'];
    ...
    for ($r = 2; $r <= $highestRow; $r++) {
        ...
        $dtAtual     = $toDateStr($get('DTAATUALIZACAO'));

        // pula linha vazia
        $isEmpty = ($codmeta===null||$codmeta==='') &&
                   ($codvendedor===null||$codvendedor==='') &&
                   ($codregiao===null||$codregiao==='') &&
                   ($seqproduto===null||$seqproduto==='') &&
                   ($meta===null) &&
                   ($segmento==='') && ($tiporetira==='') && ($categoria==='') &&
                   (!$dtAtual);
        if ($isEmpty) continue;

        // valida chave
        if ($codmeta===null || $codvendedor===null || $codregiao===null || $seqproduto===null) {
            $fail++;
            sse_send("Linha {$r}: chave incompleta (CODMETA/CODVENDEDOR/CODREGIAO/SEQPRODUTO).", 'aviso');
            continue;
        }

        // valida meta e data (se existir no oracle)
        ...
        try {
            ...
            foreach (['META','SEGMENTO','TIPORETIRA','CATEGORIA','DTAATUALIZACAO'] as $c) {
    ...
    // select megag_fn_tabs_importacao_sqlexec('megag_imp_metas') from dual;
    // ============================================================
